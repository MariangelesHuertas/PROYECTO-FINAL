...
@Injectable()
export class GoogleEmpresaStrategy extends PassportStrategy(Strategy, 'googleEmpresa') {
  constructor(private readonly usuariosService: UsuariosService, private readonly jwtService: JwtService, private readonly prisma: PrismaService) {
    super({
      ...
      callbackURL: `${process.env.URL}/auth/google/callback/empresa`,// URL DEL BACKEND
      scope: ['email', 'profile'],
    });
  }
  ...
  async validate(accessToken: string, refreshToken: string, profile: any, done: VerifyCallback): Promise<any> {
    ...
    if (!userInDb) {
      userInDb = await this.usuariosService.createG(user , "Empresa");
    }
