...
interface CalculateDifferentTimeProps {
  fecha: string; // Ahora el tipo es string para aceptar una fecha en formato ISO
}

const CalculateDifferentTime: React.FC<CalculateDifferentTimeProps> = ({ fecha }) => {
  return <span>{calcularDiferenciaFecha(fecha)}</span>;
};

export default CalculateDifferentTime;
