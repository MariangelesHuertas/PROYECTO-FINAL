...
const CustomInputNumber = ({ value, onChange }: any) => {
  ...
  return (
    <div className="relative flex items-center">
      <Input
        value={value}
        className="custom-input-number w-[111px] h-[44px] text-center text-body-md text-green32"
        onChange={(e) => onChange(Number(e.target.value))}
      />
      <img alt="Decrease" className="absolute left-2 cursor-pointer ml-[9px] w-[24px]" src={IconL} onClick={handleDecrease}/>
      <img  src={IconR} alt="Increase"
         className="absolute right-2 cursor-pointer mr-[9px] w-[24px]"
         onClick={handleIncrease}/>

      <style>{`
        .custom-input-number {
          border-radius: 4px; /* Ajustar el borde del input */
          border: 1px solid #006497; /* Color del borde */
          --active-bg: #ffffff;
